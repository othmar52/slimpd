...
/**
 * pattern "Seeed_-_10_-_Waterpumpee_With_Anthony_B__(Waanback_2002).mp3"
 * "
 */
class ArtistNumberTitleExt extends \Slimpd\Modules\Albummigrator\AbstractTests\AbstractTest {
	public $isAlbumWeight = 0.4;
	...
	public function __construct($input, &$trackContext, &$albumContext, &$jumbleJudge) {
		...
		$this->pattern = "/^" . RGX::NO_MINUS . RGX::GLUE . RGX::NUM . RGX::GLUE . RGX::NO_MINUS . RGX::EXT . "$/i";
		return $this;
	}
	...
	public function run() {
		if(preg_match($this->pattern, $this->input, $matches)) {
			...
			$this->result = 'artist-number-title-ext';
			return;
		}
		$this->result = 0;
	}
