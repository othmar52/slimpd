...
class AlbumMigrator {
	...
	public function run() {
		...
		$album->setArtistId(join(",", Artist::getIdsByString($albumArtists)));
		$album->setGenreId(join(",", Genre::getIdsByString($mergedFromTracks['genre'])));
		...
		foreach($this->tracks as $idx => $rawTagData) {
			...
			$track->setGenreId(join(",", Genre::getIdsByString($this->getMostScored($idx, 'genre'))));
			$track->setLabelId(join(",", Label::getIdsByString($this->getMostScored($idx, 'label'))));
			...
			\Slimpd\Track::ensureRecordIdExists($track->getId());
			$track->update();

			// make sure extracted images will be referenced to an album
			\Slimpd\Bitmap::addAlbumIdToTrackId($track->getId(), $albumId);#
	...
	private function updateTrackIndex($trackId, $idx) {
		...
		$trackIndex = new \Slimpd\Trackindex();
	...
	private function updateAlbumIndex($albumId) {
		...
		$albumIndex = new \Slimpd\Albumindex();
